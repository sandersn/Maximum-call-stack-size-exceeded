...
const ATTRIBUTE_INFO_BOOKING_PAGE_COVER = new ApiAttributeInfo({
...
myStory
    ...
    props: {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        attributeInfo: new ApiAttributeInfo({
            apiObjWrapper: apiObjectWrapper,
            id: 'SOME_DURATION',
            name: 'someDuration',
            canEdit: null,
...
const getStoryBookPath = (apiPath) => {
...
const getStoryBookName = (apiPath) => {
    // return last api-path item
    const lastSlashIndex = apiPath.lastIndexOf('/');
    return apiPath.substr(lastSlashIndex + 1);
};
const addStoryBookApiPreview = (apiPath) => {
 ...
 */
const addStoryBookApiPreviewsRecursively = (data, path) => {
    ...
    for (const childWrapperName of data.getChildWrapperNames()) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        addStoryBookApiPreviewsRecursively(data[childWrapperName], nextPath);
    }
};
addStoryBookApiPreviewsRecursively(storyBookApi.data);
//# sourceMappingURL=p-form-control-switch.component.stories.js.map