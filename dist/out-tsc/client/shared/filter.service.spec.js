...
describe('FilterService #needsapi', () => {
    ...
    it('toggleItem()', () => {
        ...
        const serviceContainsMemberBefore = service.isVisible(member);
        service.toggleItem(member);
        let serviceContainsMemberAfter = service.isVisible(member);
    ...
    it('someMembersAreVisible(members)', () => {
        ...
        const atLeast2TrashedMembersAreInside = () => members.filterBy(item => item.trashed).length >= 2;
    ...
    describe('isVisible()', () => {
        ...
        beforeAll(() => {
            ...
            if (!service.isVisible(shift.model))
                service.toggleItem(shift.model);
            for (const assignedMember of shift.assignedMembers.iterable()) {
                if (!service.isVisible(assignedMember))
                    service.toggleItem(assignedMember);
            }
        });
