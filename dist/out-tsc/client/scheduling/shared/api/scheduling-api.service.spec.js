...
const createFitsSearchTest = (fn, term, result) => {
...
describe('SchedulingApiService Base #needsapi', () => {
    ...
    let zone;
    let logService;
    let clientRoutingService;
    let reportFilterService;
    let schedulingFilterService;
    ...
    describe('#SchedulingApiAbsence', () => {
        ...
        describe('absence.totalDaysBetween()', () => {
            describe('2017-10-26 is day of time-shift', () => {
                const daylightSavingTimeShift = (startDate, endDate, result, addHours = 0) => {
    ...
    describe('#SchedulingApiShifts', () => {
        it(`withoutShiftModels( filterService.hiddenItems['shiftModels'] ) should not hide items`, () => {
            const filterService = new FilterService(api, pCookieService, zone, logService, clientRoutingService, reportFilterService, schedulingFilterService);
        ...
        it(`withoutAssignedMembers( filterService.hiddenItems['shiftModels'] ) should not hide items`, () => {
            ...
            filterService.initValues();
            const shifts = api.data.shifts;
            const counter = shifts.withoutAssignedMembers(filterService.hiddenItems['members'], filterService.schedulingFilterService).length;
        ...
        describe(`withoutAssignedMembers( filterService.hiddenItems['shiftModels'] )`, () => {
            it('should hide items', () => {
                ...
                expect(shifts.withoutAssignedMembers(filterService.hiddenItems['members'], options).length).toBe(shifts.length);
                ...
                if (!filterService.isVisible(someMembers))
                ...
                filterService.toggleMembers(someMembers);
                const FILTERED_SHIFTS_AMOUNT = shifts.withoutAssignedMembers(filterService.hiddenItems['members'], filterService.schedulingFilterService).length;
            ...
            it('should hide items with different showItemsWithEmptyMemberSlot states', (done) => {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                fakeApi, pCookieService, zone, logService, clientRoutingService, reportFilterService, schedulingFilterService);
                filterService.unload();
                filterService.initValues();
                ...
                shifts.createNewShift(fakeApi.data.shiftModels.get(0), new PMomentService().m(), null, (someNewShift) => {
                    ...
                    // Toggle them in filter (means hide them, because above we checked that they are visible)
                    filterService.toggleMembers(someMembers);
                    const test1ShiftsLength1 = shifts.withoutAssignedMembers(filterService.hiddenItems['members'], options).length;
                    ...
                    const test2ShiftsLength1 = shifts.withoutAssignedMembers(filterService.hiddenItems['members'], options).length;
                    options.showItemsWithEmptyMemberSlot = true;
                    const test2ShiftsLength2 = shifts.withoutAssignedMembers(filterService.hiddenItems['members'], options).length;
                    options.showItemsWithEmptyMemberSlot = true;
    ...
    const getBooking = (firstName, lastName, email, bookingNumber, participantFirstName, participantLastName) => {
    ...
    describe('#SchedulingApiBooking', () => {
        ...
        describe(`booking by ${FIRST_NAME} ${LAST_NAME} with participant Max Mayer`, () => {
            const booking = getBooking(FIRST_NAME, LAST_NAME, 'hans@dr-plano.de', 123456, 'Max', 'Mayer');
            describe('fitsSearch()', () => {
    ...
    describe('#SchedulingApiBookings', () => {
        let bookings;
        const addBooking = (list, firstName, lastName, email, bookingNumber) => {
