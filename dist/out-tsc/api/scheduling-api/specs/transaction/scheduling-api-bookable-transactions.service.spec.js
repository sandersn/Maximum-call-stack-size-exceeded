...
describe('#SchedulingApiService #needsapi', () => {
     ...
     */
    const findTransaction = (bookable, originalPsp, type, amount = null) => {
     ...
     */
    const doTransactionTests = (bookable) => {
        describe('MISC', () => {
            it('partially-pay', async () => {
                ...
                await adyenTestingUtils.createTransaction(api, bookable.val.id, transactionAmount, SchedulingApiTransactionType.PAYMENT, SchedulingApiTransactionPaymentMethodType.MISC);
            ...
            it('cannot-online-refund-because-no-online-balance', async () => {
                ...
                try {
                    await adyenTestingUtils.createTransaction(api, bookable.val.id, 1, SchedulingApiTransactionType.REFUND, SchedulingApiTransactionPaymentMethodType.ONLINE_PAYMENT);
                    fail();
                }
                catch (_a) {
            ...
            it('completely-refund', async () => {
                assumeNonNull(bookable.val);
                await adyenTestingUtils.createTransaction(api, bookable.val.id, bookable.val.currentlyPaid, SchedulingApiTransactionType.REFUND, SchedulingApiTransactionPaymentMethodType.MISC);
        ...
        describe('ONLINE_PAYMENT', () => {
            ...
            it('cannot-refund-more-than-bookables-online-balance', async () => {
                ...
                const onlinePaymentTransaction = await adyenTestingUtils.createTransaction(api, bookable.val.id, 10, SchedulingApiTransactionType.PAYMENT, SchedulingApiTransactionPaymentMethodType.ONLINE_PAYMENT);
                expect(bookable.val.currentlyPaid).toBe(10);
                ...
                await adyenTestingUtils.createTransaction(api, bookable.val.id, 10, SchedulingApiTransactionType.PAYMENT, SchedulingApiTransactionPaymentMethodType.MISC);
                expect(bookable.val.currentlyPaid).toBe(20);
                // try to refund more than available bookables online balance which should fail
                const prevTransactionCount = bookable.val.transactions.length;
                try {
                    await adyenTestingUtils.createTransaction(api, bookable.val.id, 15, SchedulingApiTransactionType.REFUND, SchedulingApiTransactionPaymentMethodType.ONLINE_PAYMENT);
                    fail();
                }
                catch (_a) {
                    // check nothing has changed
                    ...
                    await adyenTestingUtils.createTransaction(api, bookable.val.id, 10, SchedulingApiTransactionType.REFUND, SchedulingApiTransactionPaymentMethodType.MISC);
                    expect(bookable.val.currentlyPaid).toBe(0);
                }
            });
            ...
            const doRefundTests = (captured, partial) => {
            ...
            const doTransferFundsTests = (success) => {
                it(`transferFunds-${success ? 'success' : 'failed'}`, async () => {
                    ...
                    const clientsApi = testingUtils.getService(ClientsApiService);
                    ...
                    await adyenTestingUtils.sendTransferFunds(newRefund1.testingTransferFundsPSP, 0, api.data.adyenAccount.testingLiablePaymentAccount, api.data.adyenAccount.testingAccountHolderPaymentAccount, `c:${clientId};`, 'DEBIT_AGGREGATED_FEES', success);
            ...
            describe('chargeback', () => {
                ...
                it('duplicate-settlement-reports-only-create-one-chargeback', async () => {
                    ...
                    const sendSettlementDetailsReport = async () => {
                        assumeNonNull(bookable.val);
                        const clientId = api.data.id;
                        assumeNonNull(newTransaction.testingOriginalPspReference);
                        const entries = [
...
//# sourceMappingURL=scheduling-api-bookable-transactions.service.spec.js.map