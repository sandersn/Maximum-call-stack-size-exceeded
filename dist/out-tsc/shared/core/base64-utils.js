...
const supportedBase46Prefixes = /(data:image\/(png|jpeg);base64,)(.*)/;
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const getFileFormat = (dataUri) => {
    const regexReturnArray = dataUri.match(supportedBase46Prefixes);
    if (regexReturnArray === null)
        throw new Error('No match');
    if (regexReturnArray.length !== 4)
        throw new Error('Unsupported data URI format');
    if (regexReturnArray[2] !== 'png' && regexReturnArray[2] !== 'jpeg')
        throw new Error('Unsupported data URI format');
    return regexReturnArray[2];
};
const base64PrefixLength = (dataUri) => {
    const regexReturnArray = dataUri.match(supportedBase46Prefixes);
    if (regexReturnArray === null)
        throw new Error('No match');
    if (regexReturnArray.length !== 4)
...
const base64WithoutPrefix = (dataUri) => {
    const regexReturnArray = dataUri.match(supportedBase46Prefixes);
    if (regexReturnArray === null)
        throw new Error('No match');
    if (regexReturnArray.length !== 4)
...
export const getBase64Dimensions = (dataUri) => {
...
export const getBase64DimensionsAsync = (dataUri) => {
    // const base64Content = base64WithoutPrefix(dataUri);
    return getImageDimensions(dataUri);
};
export const getPngFileSize = (dataUri) => {
