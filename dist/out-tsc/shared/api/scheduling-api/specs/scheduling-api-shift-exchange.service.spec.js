...
import { SchedulingApiRightGroupRole, SchedulingApiShiftExchangeState, SchedulingApiShiftExchangeCommunicationAction, SchedulingApiShiftExchangeCommunicationState, SchedulingApiShiftsBase, SchedulingApiShiftRepetitionType, SchedulingApiAbsenceType } from '@plano/shared/api';
...
describe('#SchedulingApiServiceShiftExchange #needsapi', () => {
    ...
    describe('#shift-exchange', () => {
        ...
        const testShiftExchange = (isIllness, indisposedMemberIsAdmin, indisposedMemberPrefersSwapping, setMemberAddressedTo) => {
            describe(`(case ${isIllness} ${indisposedMemberIsAdmin} ${indisposedMemberPrefersSwapping} ${setMemberAddressedTo})`, () => {
                const ensureNoCommunicationActionsPermitted = () => {
                ...
                const getApiQueryParams = () => {
                    ...
                    if (ensureShifts.length > 0) {
                        const ensureShiftsJson = JSON.stringify(ensureShifts, (_key, value) => {
                ...
                // Prepare data
                //
                let shiftExchange = null;
                let shift = null; // And later switch it to "shift"
                let oldShiftId = null; // We start the shift-exchange with "oldShiftId"
                let shiftModel = null;
                const shiftsToSwap = new SchedulingApiShiftsBase(api, false);
                let indisposedMember;
                let admin;
                let memberAddressedTo;
                let newAssignedMember;
                it('prepare-data', (done) => {
                    executeAs(admin, () => {
                        ...
                        // prepare shifts
                        const prepareShiftsToSwap = (success) => {
                        ...
                        const prepareShifts = (success) => {
                            ...
                            api.data.shifts.createNewShift(shiftModel, pMoment.m().add(3, 'month'), getApiQueryParams(), (newShift) => {
                            ...
                            success: () => {
                                shiftExchange.loadDetailed({
                                    success: () => {
                                        ...
                                        expect(testingUtils.safeEquals(shiftExchange.memberIdAddressedTo, setMemberAddressedTo ? memberAddressedTo.id : null)).toBeTrue();
                ...
                //
                describe('shifts', () => {
                    it('first-do-communication', (done) => {
                        if (setMemberAddressedTo) {
                    ...
                    it('ensure-communications-are-reset', (done) => {
                        executeAs(newAssignedMember, () => {
                ...
                // withdraw
                //
                const closeShiftExchange = (close, done) => {
                    executeAs(isIllness ? admin : indisposedMember, () => {
                        if (close)
                ...
                it('closeShiftExchange', (done) => {
                    closeShiftExchange(true, done);
                });
                it('openShiftExchange', (done) => {
                ...
                describe('memberAddressedTo', () => {
                    if (isIllness) {
                        ...
                        const flipMemberAddressedTo = (done) => {
                            executeAs(indisposedMember, () => {
